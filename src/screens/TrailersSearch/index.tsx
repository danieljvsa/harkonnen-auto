      ...
      if(displayCurrentAddress === 'Wait, we are fetching you location...'){
        if (search != ""){
          if(item.username.toLowerCase().includes(search.toLowerCase())){
            return <CardProfileProf stars={(item.evaluationAgerage === undefined) ? '0' : item.evaluationAgerage.toString()} showNumber={true} title={item.username} image={item.image} key={index} onPress={() => goToProfDetails(item.id)} />
          } else {
            return <></>
          }
        }else{
